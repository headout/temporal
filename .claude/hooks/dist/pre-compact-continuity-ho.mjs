...
async function main() {
  ...
  let input;
  try {
    input = JSON.parse(await Promise.race([
      readStdin(),
      new Promise((_, reject) => setTimeout(() => reject(new Error("stdin timeout")), 4e3))
    ]));
  } catch {
    input = {};
  }
  ...
  if (ledgerFiles.length === 0) {
    const output = {
      result: "continue",
      systemMessage: "[PreCompact] No ledger found. Create one? /continuity-ledger"
    };
  ...
  if (input.trigger === "auto") {
    if (process.env.HEADOUT_AUTO_HANDOFF === "0") {
      console.log(JSON.stringify({ result: "continue" }));
      return;
    }
    ...
    const output = {
      result: "continue",
      systemMessage: `[PreCompact] Consider updating ledger before compacting: /continuity-ledger
Ledger: ${mostRecent}`
    };
    console.log(JSON.stringify(output));
  }
...
function appendToLedger(ledgerPath, summary) {
  try {
    ...
    const stateIdx = content.indexOf("## State");
    if (stateIdx >= 0) {
      const nextSectionIdx = content.indexOf("\n## ", stateIdx + 1);
      const stateEnd = nextSectionIdx > 0 ? nextSectionIdx : content.length;
      const stateSection = content.slice(stateIdx, stateEnd);
      const nowMatch = stateSection.match(/(\n-\s*Now:)/);
      if (nowMatch && nowMatch.index !== void 0) {
        const insertAt = stateIdx + nowMatch.index;
        content = content.slice(0, insertAt) + summary + content.slice(insertAt);
      } else {
        if (nextSectionIdx > 0) {
          content = content.slice(0, nextSectionIdx) + summary + "\n" + content.slice(nextSectionIdx);
        } else {
          content += summary;
        }
      }
