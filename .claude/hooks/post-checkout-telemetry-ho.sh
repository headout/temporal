#!/bin/bash
# Post-checkout telemetry — flushes checkpoints, clears stale fingerprints. Async.
...
exec "$(dirname "${BASH_SOURCE[0]}")/hac-dispatch-ho.sh" --git-bg --no-stdin post-checkout-telemetry-ho "$1" "$2" "$3"
